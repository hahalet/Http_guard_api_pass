...
-- HG是否开启
local hgOn = _Conf.dict_captcha:get("hgOn")
if hgOn == 1 then --判断转向模块是否开启
	if not (hgModule == "off") then
		
		--byDeny黑名单	
		if not Guard:ipInByDenyList(ip) then	

			--获取验证码
			if ngx.re.match(uri,"/get-captcha.do$","i") then
				Guard:getCaptcha()

			--验证验证码
			elseif ngx.re.match(uri,"/verify-captcha.do$","i") then
				...
				if not Guard:ipInWhiteList(ip) then
					--黑名单模块
					Guard:blackListModules(ip,reqUri,address)
