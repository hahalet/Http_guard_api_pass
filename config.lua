...
local Config = {
	-- HttpGuard是否开启
	--此项主要针对有多个虚拟主机时, 对不开 HttpGuard 的虚拟主机提高效率
	...
	limitReqModules = { state = "Off" , maxReqs = 600 , amongTime = 10, urlProtect = baseDir.."url-protect/limit.txt" },
	...
	--byPass模块
	--主要应用于网站对外开放些API接口，匹配 byPass.txt 中的 requestUri 即使在 ipInByDenyList 黑名单列表中也可通过。把需要开放的API requestUri 用正则表达式写在byPass.txt，一行一个 
	byPassModules = { state = "On" ,urlProtect = baseDir.."url-protect/byPass.txt"},
	...
	-- keySecret : 用于生成token的密码,如果上面的keyDefine为dynamic，就不需要修改
	-- urlProtect  同limitReqModules模块中的urlProtect的解释。	
	cookieModules = { state = "Off" ,verifyMaxFail = 9, keySecret = 'bGMfY2D5t3', amongTime = 60 , urlProtect = baseDir.."url-protect/cookie.txt"},
	...
	-- 验证码页面路径,一般不需要修改, 为避免缓存的问题，故使用了 .do 后缀
	...
	-- ByDeny黑名单ip文件,此名单中的IP将被直接拒绝访问,文件内容为正则表达式。
	byDenyIpModules = { state = "On", ipList = baseDir.."url-protect/byDeny_ip_list.txt" },
