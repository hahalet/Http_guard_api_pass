...
function Guard:ipInByDenyList(ip)
	if _Conf.byDenyIpModulesIsOn then --判断是否开启 byDeny 黑名单模块
		...
		if ngx.re.match(ip, _Conf.byDenyIpList) then --匹配 byDeny 黑名单列表
			self:debug("[ipInByDenyList] ip "..ip.. " match byDeny list ".._Conf.byDenyIpList,ip,"")
			return true
		else
			return false
		end	
...
--黑名单模块
function Guard:blackListModules(ip,reqUri,address)
	...
	if _Conf.dict:get(blackKey) then --判断ip是否存在黑名单字典
		...
		self:takeAction(ip,reqUri,address) --存在则执行相应动作
	end	
end
...
function Guard:redirectModules(ip,reqUri,address)
	if not (ngx.re.match(address,_Conf.byPassUrlProtect,"i")) then
			...
			if inWhiteList then --如果在白名单
				...
				if cookie_key and cookie_expire then
					...
					if tonumber(cookie_expire) > now and cookie_key == key_make then
						...
						if _Conf.dict:get(blackKey) then --如果黑名单中有该IP, 则删除
							_Conf.dict:delete(blackKey)
						end	
						return
					else
...
function Guard:verifyCaptcha(ip)
	...
	if captchaValue == postValue then --比较验证码是否相等
		...
		_Conf.dict_challenge:delete(ip.."limitreqkey") --访问记录删除
...
--执行相应动作
function Guard:takeAction(ip,reqUri,address)
	if _Conf.captchaAction then
		...
		if cookie_expire and cookie_key then
			...
			if tonumber(cookie_expire) > now and cookie_key == key_make then
				...
				return
			elseif ngx.re.match(address,_Conf.byPassUrlProtect,"i") then
				self:debug("[takeAction] cookie key is invalid, But on byPass",ip,reqUri)
				return
			else
		...
		elseif ngx.re.match(address,_Conf.byPassUrlProtect,"i") then
			self:debug("[takeAction] return captchaAction,But on byPass",ip,reqUri)
			return
		else
